...
\begin{remark}[Why Set-Level Operations?]
    Before studying subspaces, it is useful to have a calculus of \emph{subsets} of a
    vector space. The Minkowski sum $A + B = \{a + b \mid a \in A,\, b \in B\}$ and the
    scalar-set product $LA = \{\lambda a \mid \lambda \in L,\, a \in A\}$ lift the vector
    space operations from individual vectors to subsets. These operations will reappear
    throughout the subject:
    \begin{itemize}
        \item The \textbf{sum of subspaces} $U + W$ is a Minkowski sum.
        \item \textbf{Cosets} $v + W$ are translates of a subspace.
        \item The \textbf{subspace criterion} can be stated as $KU + KU = U$.
    \end{itemize}
    We establish the algebraic rules (associativity, distributivity, monotonicity) here so
    they are available when needed later. The key subtlety is that distributivity is only
    \emph{partial} for sets, unlike for individual vectors --- a point illustrated by
    explicit counterexamples below.
\end{remark}
...
\begin{proof}
    ...
    Since $L + M \neq \{0\}$, there exist $\lambda' \in L$ and $\mu' \in M$ such that $\lambda' + \mu' \neq 0$.
    Now $a - a' \in A + A = A$, so $KA = A$ gives $\frac{\lambda}{\lambda' + \mu'}(a - a') \in A$.
