...
\begin{remark}[Why Subspaces?]
    A vector space may be large or complicated, but its internal structure is revealed by its
    subspaces --- subsets that are themselves vector spaces under the inherited operations.
    Lines and planes through the origin in $\bb{R}^3$ are the prototypical examples: they are
    the ``flat'' subsets that pass through zero. Every fundamental construction in linear
    algebra --- kernel, image, span, eigenspace --- produces a subspace. Understanding
    the lattice of all subspaces of $V$ (how they intersect, sum, and nest) is therefore
    central to understanding $V$ itself.

    The \textbf{subspace criterion} below reduces the verification to a single condition:
    closure under linear combinations. This is the test used in practice.
\end{remark}
...
\begin{remark}[The Subspace Lattice $\SubspaceLattice{V}$]\label{rem:sub-lattice}
    We write $\SubspaceLattice{V}$ for the \textbf{set of all $K$-vector subspaces of $V$}, ordered by
...
\begin{example}[Linear Combinations]\label{ex:linear-combinations}
    ...
    \begin{enumerate}
        ...
        \item In $\bb{R}^3$: $(1, 2, 3) = 1 \cdot \CanonicalVector{0} + 2 \cdot \CanonicalVector{1} + 3 \cdot \CanonicalVector{2}$ where $\CanonicalVector{i}$ are standard vectors
    \end{enumerate}
\end{example}

\begin{remark}[Finiteness is Essential]\label{rem:finite-lincomb}
    Infinite series like $\sum_{i \in \bb{N}} a_i v_i$ are \textbf{not} linear combinations
    in the algebraic sense. Such series require topological structure (limits) to define.

    As a counterexample: in $K^{\bb{N}}$, let $\CanonicalVector{i} = (0, \ldots, 0, 1, 0, \ldots)$ with $1$ in position $i$.
    Then the constant sequence $(1, 1, 1, \ldots)$ is \textbf{not} a (finite) linear combination
    of the $\CanonicalVector{i}$'s, even though ``intuitively'' it equals $\sum_{i \in \bb{N}} \CanonicalVector{i}$.
\end{remark}
...
\begin{proposition}[Linear Combinations of Indicator Functions]%
    ...
    Let $I$ be any set and let $(\CanonicalVector{i})_{i \in I}$ be the indicator functions in $K^I$.
    Then the set of all linear combinations of the $\CanonicalVector{i}$'s is precisely $K^{(I)}$.
    In particular, if $I$ is infinite, then there exist elements of $K^I$ that are not
    linear combinations of the $\CanonicalVector{i}$'s.
\end{proposition}
...
\begin{proof}
    ...
    ($\subseteq$)
    Let $g$ be a linear combination of the $\CanonicalVector{i}$'s. Then
    $g = \sum_{j \in n} a_j \CanonicalVector{i_j}$ for some $n \in \bb{N}$, scalars $a_j \in K$,
    and distinct indices $i_0, \ldots, i_{n-1} \in I$. For any $k \in I$:
    \[
        g(k) = \sum_{j \in n} a_j \CanonicalVector{i_j}(k) = \sum_{j \in n} a_j \delta_{i_j k}
        =
        \begin{cases}
    ...
    Define $g = \sum_{k \in n} f(i_k) \cdot \CanonicalVector{i_k}$. For any $j \in I$:
    \[
        g(j) = \sum_{k \in n} f(i_k) \cdot \CanonicalVector{i_k}(j) = \sum_{k \in n} f(i_k) \cdot \delta_{i_k j}
        =
        \begin{cases}
    ...
    we conclude $f = g$. Hence $f$ is a linear combination of the $\CanonicalVector{i}$'s.
    ...
    By the first inclusion, $f$ is not a linear combination of the $\CanonicalVector{i}$'s.
\end{proof}
