...
\begin{example}[Linear Combinations]\label{ex:linear-combinations}
    ...
    \begin{enumerate}
        ...
        \item In $\bb{R}^3$: $(1, 2, 3) = 1 \cdot e_1 + 2 \cdot e_2 + 3 \cdot e_3$ where $e_i$ are standard vectors
    \end{enumerate}
\end{example}

\begin{remark}[Finiteness is Essential]\label{rem:finite-lincomb}
    Infinite series like $\sum_{i=1}^{\infty} a_i v_i$ are \textbf{not} linear combinations
    ...
    of the $e_i$'s, even though ``intuitively'' it equals $\sum_{i=1}^{\infty} e_i$.
\end{remark}
...
\begin{proposition}[Finite Sums Equal Minkowski Sums]\label{prop:finite-minkowski}
    ...
    \[
        \SubspaceSum{i=1}{n}{U_i} = U_1 + U_2 + \cdots + U_n
    \]
