...
\begin{example}[Spans in Coordinate Spaces]\label{ex:span-examples}
    ...
    \begin{tikzpicture}[scale=0.9, >=stealth]
        ...
        \begin{scope}[xshift=7cm]
            % Plane drawn as a receding-floor parallelogram in perspective:
            % bottom edge slopes slightly down-right, sides lean slightly right going up
            \fill[purple!20, opacity=0.5] (-1.8,-0.3) -- (2.0,-0.7) -- (2.4,1.3) -- (-1.4,1.7) -- cycle;
            \draw[->, thick, blue] (0,0) -- (1.5,0.3) node[right] {$v_1$};
            \draw[->, thick, red] (0,0) -- (0.5,1.2) node[above] {$v_2$};
            \fill (0,0) circle (2pt) node[below left] {$0$};
            \node at (0.5,-1.2) {Plane through origin};
        \end{scope}
    \end{tikzpicture}
    \end{center}
\end{example}
...
\begin{example}[Spans in $\bb{R}^3$]\label{ex:span-R3}
    In $\bb{R}^3$, consider the standard basis vectors $e_0 = (1,0,0)$, $e_1 = (0,1,0)$, $e_2 = (0,0,1)$:
    \begin{enumerate}
        \item \textbf{Line}: $\Span[\bb{R}]{\{e_2\}} = \Set{(0, 0, z) \mid z \in \bb{R}}$ (the $z$-axis)
        \item \textbf{Plane}: $\Span[\bb{R}]{\{e_0, e_1\}} = \Set{(x, y, 0) \mid x, y \in \bb{R}}$ (the $xy$-plane)
        \item \textbf{Whole space}: $\Span[\bb{R}]{\{e_0, e_1, e_2\}} = \bb{R}^3$
    \end{enumerate}
    ...
    \begin{tikzpicture}[scale=1.2,
        ...
        \node[blue] at (1.5, 0, 0.3) {$\Span{e_0, e_1}$};

        % z-axis (span of e3)
        \draw[thick, red] (0,0,-1.5) -- (0,0,1.5);
        \node[red] at (0.4,0,1.5) {$\Span{e_2}$};
...
\begin{proposition}[Constructive Characterization of Span]\label{prop:span-constructive}
    ...
    \[
        \Span[K]{S} = \Set{\sum_{i \in n} a_i v_i \mid n \in \bb{N}, a_i \in K, v_i \in S}
    \]
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item \textbf{Closure}: If $x = \sum_{i \in n} a_i v_i$ and $y = \sum_{j \in m} b_j w_j$ are in $L$, then
              for $\alpha, \beta \in K$:
              \[
                  \alpha x + \beta y = \sum_{i \in n} \alpha a_i v_i + \sum_{j \in m} \beta b_j w_j \in L
              \]
...
\begin{proof}
    ...
    (2) Every linear combination of $0$ has the form $\sum_{i \in n} a_i \cdot 0 = 0$.
    Thus $\Span[K]{\{0\}} = \{0\}$.
\end{proof}
...
\begin{proposition}[$\operatorname{span}_K(\{e_i\}_{i \in I}) = K^{(I)}$]\label{prop:span-ei-finite-support}
    For any index set $I$, the span of the indicator functions equals the space of
    functions with finite support:
    \[
        \Span[K]{\{e_i\}_{i \in I}} = K^{(I)}
    \]
\end{proposition}

\begin{proof}
    This is an immediate reformulation of Proposition~\ref{prop:lincomb-finite-support} in the
    language of span: the set of all linear combinations of the $e_i$'s is precisely $K^{(I)}$,
    and by the constructive characterization of span (Proposition~\ref{prop:span-constructive}),
    this set equals $\Span[K]{\{e_i\}_{i \in I}}$.
\end{proof}

\begin{corollary}\label{cor:span-ei-not-KI}
    If $I$ is infinite, then $\Span[K]{\{e_i\}_{i \in I}} = K^{(I)} \subsetneq K^I$.
    In particular, the indicator functions $(e_i)_{i \in I}$ do \textbf{not} generate $K^I$.
\end{corollary}

\begin{proof}
    By Proposition~\ref{prop:span-ei-finite-support}, $\Span[K]{\{e_i\}_{i \in I}} = K^{(I)}$.
    Since $I$ is infinite, the constant-$1$ function $f(i) = 1$ for all $i \in I$ lies in
    $K^I \setminus K^{(I)}$ (as shown in Proposition~\ref{prop:lincomb-finite-support}),
    so $K^{(I)} \subsetneq K^I$.
\end{proof}
