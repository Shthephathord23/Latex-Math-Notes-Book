...
\begin{example}[Linear Independence and Dependence]\label{ex:lin-indep-dep}
    ...
    \begin{itemize}
        ...
        \item $\{1, X, X^2\}$ in $K[X]$
        \item $\{\CanonicalVector{0}, \CanonicalVector{1}, \CanonicalVector{2}\}$ (standard basis) in $\bb{R}^3$
    \end{itemize}
...
\begin{example}[Linear Independence in $\bb{R}^3$]\label{ex:lin-indep-R3}
    ...
    \begin{itemize}
        \item $\{\CanonicalVector{0}, \CanonicalVector{1}, \CanonicalVector{2}\} = \{(1,0,0), (0,1,0), (0,0,1)\}$ --- the standard basis
        \item $\{(1,1,0), (1,0,1), (0,1,1)\}$ --- three non-coplanar vectors
    \end{itemize}

    \textbf{Dependent (more than $\dim = 3$ vectors)}:
    \begin{itemize}
        \item $\{\CanonicalVector{0}, \CanonicalVector{1}, \CanonicalVector{2}, (1,1,1)\}$ --- any 4 vectors in $\bb{R}^3$ are dependent
    \end{itemize}
...
\begin{example}[Dependence Examples]\label{ex:dependence}
    In $\bb{R}^3$ with standard basis $\{\CanonicalVector{0}, \CanonicalVector{1}, \CanonicalVector{2}\}$:
    \begin{itemize}
        \item $(1, 2, 3)$ is linearly dependent on $\{\CanonicalVector{0}, \CanonicalVector{1}, \CanonicalVector{2}\}$ because
              $(1, 2, 3) = 1 \cdot \CanonicalVector{0} + 2 \cdot \CanonicalVector{1} + 3 \cdot \CanonicalVector{2} \in \Span[\bb{R}]{\{\CanonicalVector{0}, \CanonicalVector{1}, \CanonicalVector{2}\}}$.
        \item $\CanonicalVector{0}$ is linearly \emph{independent} of $\{\CanonicalVector{1}, \CanonicalVector{2}\}$ because
              $\CanonicalVector{0} = (1, 0, 0) \notin \Span[\bb{R}]{\{\CanonicalVector{1}, \CanonicalVector{2}\}} = \Set{(0, y, z) \mid y, z \in \bb{R}}$.
    \end{itemize}
\end{example}
...
\begin{theorem}[Characterizations of Linear Independence]\label{thm:lin-indep-equiv}
    ...
    \begin{enumerate}
        ...
        \item[(iv)] $0$ has unique representation (i.e., if $0 = \sum_{i \in n} a_i \ell_i$ with
...
\begin{proof}
    ...
    Suppose $0 = \sum_{i \in n} a_i \ell_i$ with $a_i \neq 0$ and $n \geq 1$.
    Then $\ell_0 = -\frac{1}{a_0}\sum_{i \in n,\, i \neq 0} a_i \ell_i \in \Span[K]{L \setminus \{\ell_0\}}$,
    contradicting (i').
    ...
    If $v \notin L \setminus \{v\}$ (the summands), this is a nontrivial relation, contradicting (iv).
\end{proof}
...
\begin{proposition}[Zero Vector Implies Dependence]\label{prop:zero-dependent}
    If $0 \in L$, then $L$ is linearly dependent.
\end{proposition}

\begin{proof}
    The relation $1 \cdot 0 = 0$ is a nontrivial linear combination
    (one term with nonzero coefficient), violating condition (iv).
\end{proof}
...
\begin{proposition}[Finite Character of Linear Independence]\label{prop:lin-indep-finite-char}
    A set $L \subseteq V$ is linearly independent if and only if every finite subset of $L$
    is linearly independent.
\end{proposition}

\begin{proof}
    ($\Rightarrow$): Proposition~\ref{prop:subset-lin-indep}.
    ($\Leftarrow$): Every nontrivial linear combination $\sum_{i \in n} a_i \ell_i = 0$
    involves only finitely many elements $\{\ell_0, \ldots, \ell_{n-1}\} \subseteq L$.
    If this finite subset is linearly independent, all $a_i = 0$, contradicting nontriviality.
\end{proof}
