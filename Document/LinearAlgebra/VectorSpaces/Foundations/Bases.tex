...
\begin{remark}[Why Bases?]
    A basis is the central concept of linear algebra. It is a set of vectors that is
    simultaneously linearly independent (no redundancy) and generating (no gaps). Equivalently,
    it is a set in which every vector in the space has a \textbf{unique} expression as a
    finite linear combination.

    The existence of a basis (guaranteed by Zorn's lemma) has far-reaching consequences:
    \begin{itemize}
        \item It reduces the study of an arbitrary vector space to the study of the
              coordinate space $K^{(I)}$, since every space with a basis of cardinality $|I|$
              is isomorphic to $K^{(I)}$.
        \item It reduces the study of linear maps to the study of their values on basis
              vectors (the universal property, Theorem~\ref{thm:vector-space-universal}).
        \item It produces a numerical invariant --- the \textbf{dimension} --- that classifies
              vector spaces up to isomorphism.
    \end{itemize}
    The fact that any two bases of the same space have the same cardinality
    (Theorem~\ref{thm:dim-invariance}) is what makes dimension well-defined.
    This is ultimately a consequence of the Steinitz exchange lemma
    (Theorem~\ref{thm:steinitz}).
\end{remark}
...
\begin{example}[Standard Bases]\label{ex:standard-bases}
    ...
    \begin{enumerate}
        ...
        \item $\{\CanonicalVector{0}, \ldots, \CanonicalVector{n-1}\}$ is the standard basis for $K^n$
    \end{enumerate}
    ...
    \begin{tikzpicture}[scale=1.0, >=stealth]
        ...
        \begin{scope}
            ...
            \draw[->, very thick, blue] (0,0) -- (1,0) node[below right] {$\CanonicalVector{0}$};
            \draw[->, very thick, red] (0,0) -- (0,1) node[above left] {$\CanonicalVector{1}$};
...
\begin{example}[Bases in $\bb{R}^3$]\label{ex:bases-R3}
    ...
    \begin{itemize}
        \item $\{\CanonicalVector{0}, \CanonicalVector{1}, \CanonicalVector{2}\} = \{(1,0,0), (0,1,0), (0,0,1)\}$ --- standard basis
    ...
    \begin{itemize}
        \item $\{\CanonicalVector{0}, \CanonicalVector{1}, \CanonicalVector{2}, (1,1,1)\}$ --- 4 vectors in $\bb{R}^3$, so dependent
    \end{itemize}

    \textbf{Not a basis (independent but not spanning)}:
    \begin{itemize}
        \item $\{\CanonicalVector{0}, \CanonicalVector{1}\}$ --- only spans the $xy$-plane, not all of $\bb{R}^3$
    \end{itemize}
    ...
    \begin{tikzpicture}[scale=1.0,
        ...
        \draw[->, very thick, blue] (0,0,0) -- (1.5,0,0) node[right] {$\CanonicalVector{0}$};
        \draw[->, very thick, red] (0,0,0) -- (0,1.5,0) node[above right] {$\CanonicalVector{1}$};
        \draw[->, very thick, green!60!black] (0,0,0) -- (0,0,1.5) node[above] {$\CanonicalVector{2}$};
...
\begin{definition}[Complementary Subspaces]\label{def:complement}
    Let $U \Subspace[K] V$. A subspace $W \Subspace[K] V$ is a \textbf{complement} of $U$ in $V$
    if $V = U \DirectSum W$, i.e., if:
    \begin{enumerate}
        \item $U + W = V$ (the subspaces span $V$)
        \item $U \Intersect W = \{0\}$ (the subspaces meet trivially)
    \end{enumerate}
    We also say $U$ and $W$ are \textbf{complementary subspaces} of $V$.
\end{definition}

\begin{proposition}[Existence of Complements]\label{prop:complement-existence}
    Every subspace $U \Subspace[K] V$ has a complement in $V$.
\end{proposition}

\begin{proof}
    Let $B_U$ be a basis for $U$. By the Basis Extension Theorem (Theorem~\ref{thm:basis-extension}),
    $B_U$ extends to a basis $B = B_U \Union C$ for $V$, where $B_U \Intersect C = \emptyset$ (take $C = \SetMinus{B}{B_U}$).
    
    Let $W = \Span[K]{C}$. We show $V = U \DirectSum W$:
    \begin{itemize}
        \item \textbf{$U + W = V$}: Since $B = B_U \Union C$ spans $V$, and $B_U$ spans $U$ while $C$ spans $W$,
              every $v \in V$ can be written as a sum of elements from $U$ and $W$.
        \item \textbf{$U \Intersect W = \{0\}$}: Let $v \in U \Intersect W$. Then $v = \sum_{b \in F_U} a_b b$
              for some finite $F_U \subseteq B_U$, and $v = \sum_{c \in F_C} d_c c$ for some finite $F_C \subseteq C$.
              Thus $\sum_{b \in F_U} a_b b - \sum_{c \in F_C} d_c c = 0$.
              Since $B = B_U \Union C$ is linearly independent and $B_U \Intersect C = \emptyset$,
              all coefficients are zero. Hence $v = 0$.
    \end{itemize}
\end{proof}

\begin{corollary}[Non-Uniqueness of Complements]\label{cor:complement-non-unique}
    Complements are generally \textbf{not unique}. Different extensions of a basis for $U$ yield
    different complements.
\end{corollary}

\begin{example}[Multiple Complements]\label{ex:complements}
    In $\bb{R}^2$, let $U = \Span[\bb{R}]{(1, 0)}$ (the $x$-axis).
    \begin{itemize}
        \item $W_1 = \Span[\bb{R}]{(0, 1)}$ (the $y$-axis) is a complement of $U$
        \item $W_2 = \Span[\bb{R}]{(1, 1)}$ (the line $y = x$) is also a complement of $U$
        \item In fact, $\Span[\bb{R}]{(a, b)}$ is a complement of $U$ for any $b \neq 0$
    \end{itemize}
    All these give $\bb{R}^2 = U \DirectSum W_i$, illustrating non-uniqueness.
\end{example}
...
\begin{proof}
    Consider the poset
    \[
        \mathcal{P} = \Set{(L_0, S_0) \mid L_0 \subseteq L, S_0 \subseteq S, \Exists :{g \colon L_0 \to S_0}{g \text{ a bijection}},
...
\begin{proposition}[Dimensions of Finite-Dimensional Spaces]\label{prop:dim-finite}
    The following hold:
    \begin{enumerate}
        \item $\Dim[K]{K^n} = n$, since $\{\CanonicalVector{0}, \ldots, \CanonicalVector{n-1}\}$ is a basis.
...
\begin{proof}
    ...
    \textbf{(1)}: The standard vectors $\CanonicalVector{i} = (0, \ldots, 0, 1, 0, \ldots, 0)$ with $1$ in position $i$
    span $K^n$ (any $(a_0, \ldots, a_{n-1}) = \sum_{i \in n} a_i \CanonicalVector{i}$) and are linearly independent
...
\begin{proposition}[All Complements Have the Same Dimension]\label{prop:complement-dim}
    Let $U \Subspace[K] V$. If $W_1$ and $W_2$ are both complements of $U$ in $V$
    (i.e.\ $V = U \DirectSum W_1 = U \DirectSum W_2$), then
    $\Dim[K]{W_1} = \Dim[K]{W_2}$.
\end{proposition}

\begin{proof}
    The additive identity
    $\Dim[K]{U} + \Dim[K]{W_1} = \Dim[K]{V} = \Dim[K]{U} + \Dim[K]{W_2}$
    (Corollary~\ref{cor:direct-sum-dim}) does \emph{not} permit cancelling $\Dim[K]{U}$:
    for infinite cardinals, $\kappa + \mu = \kappa + \nu$ does not imply $\mu = \nu$
    (for instance $\aleph_0 + 1 = \aleph_0 + 2$). Instead we construct an explicit bijection
    between $W_1$ and $W_2$ that carries a basis to a basis.

    \textbf{The projection onto $W_1$ along $U$.}
    Since $V = U \DirectSum W_1$, Theorem~\ref{thm:direct-sum-equiv} gives every $v \in V$
    a \emph{unique} representation $v = u + w$ with $u \in U$ and $w \in W_1$.
    Define a function $\varphi \colon V \to W_1$ by $\varphi(v) = w$, the $W_1$-component of $v$.

    \emph{$\varphi$ preserves the vector space operations.}
    Let $v = u + w$ and $v' = u' + w'$ be the decompositions of $v, v' \in V$
    (so $u, u' \in U$ and $w, w' \in W_1$), and let $\lambda \in K$. Then
    \[
        v + v' = (u + u') + (w + w'), \qquad \lambda v = (\lambda u) + (\lambda w),
    \]
    where $u + u', \lambda u \in U$ and $w + w', \lambda w \in W_1$.
    By uniqueness of the decomposition these \emph{are} the representations of $v + v'$ and
    $\lambda v$, so
    \[
        \varphi(v + v') = w + w' = \varphi(v) + \varphi(v'), \qquad
        \varphi(\lambda v) = \lambda w = \lambda\, \varphi(v).
    \]

    \emph{Two immediate values.} If $w \in W_1$ then $w = 0 + w$ is its decomposition, so
    $\varphi(w) = w$. If $u \in U$ then $u = u + 0$, so $\varphi(u) = 0$.
    Conversely, $\varphi(v) = 0$ means the $W_1$-component of $v$ vanishes, i.e.\ $v = u \in U$.
    Hence
    \begin{equation}
        \varphi(v) = 0 \iff v \in U. \tag{$\ast$}\label{eq:complement-kernel}
    \end{equation}

    \textbf{The restriction of $\varphi$ to $W_2$ is a bijection onto $W_1$.}
    Let $g \defeq \Restriction{\varphi}[W_2] \colon W_2 \to W_1$.

    \emph{Injective.} Suppose $w \in W_2$ with $g(w) = 0$. By~\eqref{eq:complement-kernel},
    $w \in U$, so $w \in U \Intersect W_2 = \{0\}$ (as $V = U \DirectSum W_2$), giving $w = 0$.

    \emph{Surjective.} Let $w_1 \in W_1$. Since $V = U \DirectSum W_2$, write
    $w_1 = u + w_2$ with $u \in U$ and $w_2 \in W_2$. Applying $\varphi$ and using
    $\varphi(u) = 0$ together with $\varphi(w_1) = w_1$,
    \[
        w_1 = \varphi(w_1) = \varphi(u) + \varphi(w_2) = 0 + g(w_2) = g(w_2),
    \]
    so $w_1$ lies in the image of $g$.

    \textbf{Transporting a basis.}
    Let $B$ be a basis of $W_2$, and set $\DirectImage{g}{B} = \Set{g(b) \mid b \in B}$.
    We show $\DirectImage{g}{B}$ is a basis of $W_1$ with $\Card{\DirectImage{g}{B}} = \Card{B}$.

    Since $g$ is injective, the vectors $g(b)$ for $b \in B$ are pairwise distinct, so
    $b \mapsto g(b)$ is a bijection of $B$ onto $\DirectImage{g}{B}$ and
    $\Card{\DirectImage{g}{B}} = \Card{B}$.

    \emph{Spanning.} Let $w_1 \in W_1$. By surjectivity $w_1 = g(w_2)$ for some $w_2 \in W_2$,
    and $w_2 = \sum_{i \in F} \lambda_i b_i$ for a finite $F$ and distinct $b_i \in B$.
    As $g$ preserves the operations,
    \[
        w_1 = g\!\left(\sum_{i \in F} \lambda_i b_i\right) = \sum_{i \in F} \lambda_i\, g(b_i)
        \in \Span[K]{\DirectImage{g}{B}}.
    \]

    \emph{Linear independence.} Suppose $\sum_{i \in F} \mu_i\, g(b_i) = 0$ for a finite $F$
    and distinct $b_i \in B$. Then $g\!\left(\sum_{i \in F} \mu_i b_i\right) = 0$, so
    $\sum_{i \in F} \mu_i b_i = 0$ by injectivity of $g$, whence all $\mu_i = 0$ by linear
    independence of $B$.

    Therefore $\DirectImage{g}{B}$ is a basis of $W_1$, and
    \[
        \Dim[K]{W_1} = \Card{\DirectImage{g}{B}} = \Card{B} = \Dim[K]{W_2}. \qedhere
    \]
\end{proof}

\begin{definition}[Codimension]\label{def:codimension}
    Let $U \Subspace[K] V$. The \textbf{codimension} of $U$ in $V$ is
    \[
        \Codim[K]{U}{V} \defeq \Dim[K]{W}
    \]
    where $W$ is any complement of $U$ in $V$ (well-defined by
    Proposition~\ref{prop:complement-dim}). Equivalently,
    $\Codim[K]{U}{V} = \Dim[K]{V} - \Dim[K]{U}$ when $V$ is finite-dimensional.
\end{definition}
...
\begin{theorem}[Inclusion-Exclusion for Dimension]\label{thm:dim-inclusion-exclusion}
    For a finite family $(U_0, \ldots, U_{n-1})$ of finite dimensional subspaces:
    \[
        \Dim[K]{\sum_{i \in n} U_i} = \sum_{\emptyset \neq J \subseteq \{0,\ldots,n-1\}} (-1)^{|J|+1} \Dim[K]{\BigIntersect[j \in J] U_j}
    \]
\end{theorem}
...
\begin{proof}
    ...
    For the inductive step, apply Grassmann to $U_{n-1}$ and $\sum_{i \in n-1} U_i$, then use the
    identity $U_{n-1} \Intersect \sum_{i \in n-1} U_i = \sum_{i \in n-1} (U_{n-1} \Intersect U_i)$ (which holds for
...
\begin{proof}
    ...
    \textit{Surjectivity}: Given $(a_0, \ldots, a_{n-1}) \in (K \setminus \{0\})^n$ and $\{b_0, \ldots, b_{n-1}\} \in \binom{B}{n}$,
    define $v = \sum_{i \in n} a_i b_i$. Then $\phi(v) = ((a_0, \ldots, a_{n-1}), \{b_0, \ldots, b_{n-1}\})$.
...
\begin{proof}
    ...
    are linearly independent (a nontrivial relation $\sum_{i \in n} c_i e^{a_i x} = 0$
