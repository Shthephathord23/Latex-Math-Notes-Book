...
\begin{proof}
    ...
    Suppose $\sum_{i=1}^{n} a_i T(c_i) = 0$ for $c_i \in C$.
    Then $T\left(\sum_{i=1}^{n} a_i c_i\right) = 0$, so $\sum_{i=1}^{n} a_i c_i \in \Ker{T} = \Span[K]{B_K}$.
    
    Write $\sum_{i=1}^{n} a_i c_i = \sum_{j=1}^{m} b_j k_j$ for some $k_j \in B_K$.
    Then $\sum_{i=1}^{n} a_i c_i - \sum_{j=1}^{m} b_j k_j = 0$.
