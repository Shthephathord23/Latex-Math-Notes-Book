...
\begin{proof}
    ...
    Since $\Chart{\mathcal{B}_U}$ and $\Chart{\mathcal{B}_V}$ are isomorphisms, pre- and
...
\label{ex:cob-endomorphism}
    ...
    $\mathcal{B} = (\CanonicalVector{0}, \CanonicalVector{1})$ and $\mathcal{C} = (\CanonicalVector{0} + \CanonicalVector{1},\, \CanonicalVector{1})$.

    \textit{Step 1 --- $\CoordMatrix[\mathcal{B}][\mathcal{B}]{T}$}: Apply $T$ to $\mathcal{B}$:
    $T(\CanonicalVector{0}) = (2,1)$ and $T(\CanonicalVector{1}) = (1,0)$, so
    \[
    ...
    $\mathcal{B}$ in $\mathcal{C}$: $\CanonicalVector{0} = (\CanonicalVector{0}+\CanonicalVector{1}) - \CanonicalVector{1}$, $\CanonicalVector{1} = \CanonicalVector{1}$, so
    \[
    ...
    $\mathcal{C}$ in $\mathcal{B}$: $\CanonicalVector{0}+\CanonicalVector{1} = \CanonicalVector{0}+\CanonicalVector{1}$, $\CanonicalVector{1} = \CanonicalVector{1}$, so
    \[
    ...
    \textit{Direct verification}: $T(\CanonicalVector{0}+\CanonicalVector{1}) = (3,1) = 3(\CanonicalVector{0}+\CanonicalVector{1}) + (-2)\CanonicalVector{1}$... let
    us instead verify via the coordinate formula.
    $\CoordVec[\mathcal{C}]{\CanonicalVector{0}+\CanonicalVector{1}} = \begin{psmallmatrix}1\\0\end{psmallmatrix}$,
    $\CoordVec[\mathcal{C}]{T(\CanonicalVector{0}+\CanonicalVector{1})} = \CoordVec[\mathcal{C}]{(3,1)}$.
    Now $(3,1) = 3(\CanonicalVector{0}+\CanonicalVector{1}) - 2 \CanonicalVector{1}$, so
    ...
    Direct check: first column is $\CoordVec[\mathcal{C}]{T(\CanonicalVector{0}+\CanonicalVector{1})}
    = \begin{psmallmatrix}3\\-2\end{psmallmatrix}$ $\checkmark$; second column is
    $\CoordVec[\mathcal{C}]{T(\CanonicalVector{1})} = \CoordVec[\mathcal{C}]{(1,0)}
