...
\begin{proof}[Proof (Categorical)]
    ...
    We construct $\phi: \ExtDirectSum{j \in n}{V_j} \to \DirectProd{i \in n}{V_i}$ using 
    both universal properties.
    
    \textit{Step 3a}: By the coproduct universal property, to define $\phi$ it suffices to specify 
    the compositions $\phi \circ \iota_j: V_j \to \DirectProd{}{V_i}$ for each $j \in \{0, \ldots, n-1\}$.
    ...
    Define:
    \[
        \psi = \sum_{k \in n} \iota_k \circ \pi_k: \DirectProd{}{V_i} \to \ExtDirectSum{}{V_j}
    \]
    ...
    \textit{Computation}: For each $j \in \{0, \ldots, n-1\}$:
    \begin{align*}
        (\psi \circ \phi) \circ \iota_j 
        &= \psi \circ (\phi \circ \iota_j) && \text{(associativity)} \\
        &= \left(\sum_{k \in n} \iota_k \circ \pi_k\right) \circ (\phi \circ \iota_j) && \text{(definition of $\psi$)} \\
        &= \sum_{k \in n} \left(\iota_k \circ \pi_k \circ \phi \circ \iota_j\right) && \text{(distributivity in additive category)} \\
        &= \sum_{k \in n} \left(\iota_k \circ (\pi_k \circ \phi \circ \iota_j)\right) && \text{(associativity)} \\
        &= \sum_{k \in n} \iota_k \circ \delta_{kj} && \text{(Step 3c)} \\
    ...
    \textit{Computation}: For each $i \in \{0, \ldots, n-1\}$:
    \begin{align*}
        \pi_i \circ (\phi \circ \psi)
        &= \pi_i \circ \phi \circ \left(\sum_{k \in n} \iota_k \circ \pi_k\right) && \text{(definition of $\psi$)} \\
        &= \sum_{k \in n} \left(\pi_i \circ \phi \circ \iota_k \circ \pi_k\right) && \text{(distributivity)} \\
        &= \sum_{k \in n} \left((\pi_i \circ \phi \circ \iota_k) \circ \pi_k\right) && \text{(associativity)} \\
        &= \sum_{k \in n} \delta_{ik} \circ \pi_k && \text{(Step 3c)} \\
    ...
    \begin{itemize}
        \item $\psi = \sum_{k \in n} \iota_k \circ \pi_k$ is a finite sum of morphisms
...
\label{thm:prod-coprod-dim}
    ...
    \begin{enumerate}
        ...
        \item For finite $I$ with $\Card{I} = n \in \bb{N}$: 
              $\Dim[K]{\DirectProd{i \in n}{V_i}} = \sum_{i \in n} d_i$
...
\begin{proof}
    ...
    \textit{Linear independence}: Suppose $\sum_{j \in m} c_j u_{\alpha_j} = 0$ for distinct $\alpha_0, \ldots, \alpha_{m-1}$.
    Evaluating at coordinates $\nu(0), \nu(1), \ldots, \nu(m-1)$:
    \[
        \sum_{j \in m} c_j \alpha_j^n = 0 \quad \text{for } n = 0, 1, \ldots, m-1
    \]
    ...
    \[
        \det(\mathbf{V}) = \prod_{0 \leq r < s \leq m-1} (\alpha_s - \alpha_r) \neq 0
    \]
...
\begin{proof}
    Let $\Dim[K]{U} = n$ with basis $\mathcal{B} = (u_0, \ldots, u_{n-1})$.
    ...
    \]
    The total support is $F = \bigcup_{k \in n} F_k$, which is a finite union of finite sets, hence finite.
    
    For any $i \notin F$ and any $u = \sum_{k \in n} a_k u_k \in U$:
    \begin{align*}
        (\pi_i \circ T)(u) &= \pi_i\left(T\left(\sum_{k \in n} a_k u_k\right)\right) \\
        &= \pi_i\left(\sum_{k \in n} a_k T(u_k)\right) && \text{($T$ is linear)} \\
        &= \sum_{k \in n} a_k \cdot \pi_i(T(u_k)) && \text{($\pi_i$ is linear)} \\
        &= \sum_{k \in n} a_k \cdot 0 = 0 && \text{($i \notin F_k$ for all $k$)}
    \end{align*}
    ...
    that $F = \bigcup_{k \in n} F_k$ is a finite union. For infinite-dimensional $U$, this union
...
\label{cor:coordinate-rep}
    For any basis $B$ of $V$, the coordinate map $\Chart{B}: V \to K^{(B)}$ is an isomorphism.
